...
class FSMStateGroup(StatesGroup):
    fill_name = State()
    fill_age = State()
    fill_gender = State()
    upload_photo = State()
...
async def process_start_fill(message : Message, state: FSMContext):
    await message.answer(text = 'Введите свое имя')
    await state.set_state(FSMStateGroup.fill_name)

@dp.message(StateFilter(FSMStateGroup.fill_name), F.text.isalpha())
async def process_name_fill(message : Message, state: FSMContext):
    await state.update_data(name = message.text)
    await message.answer(text = 'Спасибо! А теперь введите свой возраст')
    await state.set_state(FSMStateGroup.fill_age)

@dp.message(StateFilter(FSMStateGroup.fill_name))
...
@dp.message(StateFilter(FSMStateGroup.fill_age), lambda x: F.text.isdigit() and 4 <= int(x.text) <= 100)
async def process_age_fill(message : Message, state: FSMContext):
    ...
    await message.answer(text = 'Спасибо! А теперь введите свой гендер', reply_markup=markup)
    await state.set_state(FSMStateGroup.fill_gender)

@dp.message(StateFilter(FSMStateGroup.fill_age))
async def warning_not_age(message: Message):
    await message.answer(
        text='Возраст должен быть целым числом от 4 до 100\n\n'
             'Попробуйте еще раз\n\nЕсли вы хотите прервать '
             'заполнение анкеты - отправьте команду /cancel'
    )

@dp.message(StateFilter(FSMStateGroup.fill_gender), F.data.in_(['male', 'female', 'undefined_gender']))
async def process_gender_fill(call: CallbackQuery, state : FSMContext):
    await state.update_data(gender = call.data)
    await call.message.delete()
    await call.message.answer(
        text='Спасибо! А теперь загрузите, пожалуйста, ваше фото'
    )
    await state.set_state(FSMStateGroup.upload_photo)

@dp.message(StateFilter(FSMStateGroup.fill_gender))
async def warning_gender_fill(message: Message):
    await message.answer(
...
@dp.message(StateFilter(FSMStateGroup.upload_photo),
            F.photo[-1].as_('largest_photo'))
async def process_photo_fill(message: Message, state: FSMContext, largest_photo: PhotoSize):
    ...
    await state.set_state(FSMStateGroup.fill_education)
...
@dp.callback_query(StateFilter(FSMStateGroup.fill_education),
...
async def process_education_press(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FSMStateGroup.fill_wish_news)

@dp.message(StateFilter(FSMStateGroup.fill_education))
...
@dp.callback_query(StateFilter(FSMStateGroup.fill_wish_news),
...
@dp.message(StateFilter(FSMStateGroup.fill_wish_news))
